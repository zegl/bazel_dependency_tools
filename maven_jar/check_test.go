...
func TestNewestAvailable(t *testing.T) {
	newest, sha1, err := NewestAvailable("com.google.zxing:core:3.3.0", nil)
	assert.Nil(t, err)
...
func TestNewestAvailableOddJarSha1(t *testing.T) {
	newest, sha1, err := NewestAvailable("mx4j:mx4j-tools:3.0.1", nil)
	assert.Nil(t, err)
...
func TestNewestAvailableOroOro(t *testing.T) {
	newest, sha1, err := NewestAvailable("oro:oro:2.0.6", nil)
	assert.Nil(t, err)
